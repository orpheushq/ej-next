...
export const withLocaleRedirection: MiddlewareFactory = (next: NextMiddleware) => {
  return async (req: NextRequest, _next: NextFetchEvent) => {
    const cookieName = 'i18next'
    const res = await next(req, _next)

    // TODO: find a better way to exclude images and assets from being redirected
    if (req.nextUrl.pathname.match('/((?!api|_next/static|auth|protected|_next/image|assets|favicon.ico|sw.js).*)') != null) {
      let lng: string | null = ""
